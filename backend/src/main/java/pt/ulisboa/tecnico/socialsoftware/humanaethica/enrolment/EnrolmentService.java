...
public class EnrolmentService {
    ...
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public List<EnrolmentDto> getEnrolmentsByActivity(Integer activityId) {
        return enrolmentRepository.getEnrolmentsByActivityId(activityId).stream()
                .map(enrolment-> new EnrolmentDto(enrolment, true, true))
                .sorted(Comparator.comparing(EnrolmentDto::getId))
                .toList();
    }
